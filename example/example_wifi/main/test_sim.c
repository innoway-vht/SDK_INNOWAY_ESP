...
static void simcom_innoway_mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
...
void app_main(void)
{
	...
	simcom_innoway_init(ECHO_UART_PORT_NUM, ECHO_TEST_TXD, ECHO_TEST_RXD);
	ESP_LOGI(TAG, "Firmware Version: %s", simcom__get_firmware_version());
	if (simcom_is_registered())
	{
